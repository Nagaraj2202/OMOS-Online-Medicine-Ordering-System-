...
class SystemSettings extends DBConnection{
	...
	function update_settings_info(){
		...
		if(!empty($_FILES['lev']['tmp_name'])){
			$ext = pathinfo($_FILES['lev']['name'], PATHINFO_EXTENSION);
			$fname = "uploads/lev.png";
			$accept = array('image/jpeg','image/png');
			if(!in_array($_FILES['lev']['type'],$accept)){
				$err = "Image file type is invalid";
			}
			if($_FILES['lev']['type'] == 'image/jpeg')
				$uploadfile = imagecreatefromjpeg($_FILES['lev']['tmp_name']);
			elseif($_FILES['lev']['type'] == 'image/png')
				$uploadfile = imagecreatefrompng($_FILES['lev']['tmp_name']);
			if(!$uploadfile){
				$err = "Image is invalid";
			}
			list($width,$height) = getimagesize($_FILES['lev']['tmp_name']);
			$temp = imagescale($uploadfile,$width,$height);
			if(is_file(base_app.$fname))
			unlink(base_app.$fname);
			$upload =imagepng($temp,base_app.$fname);
			if($upload){
				if(isset($_SESSION['system_info']['lev'])){
					$qry = $this->conn->query("UPDATE system_info set meta_value = CONCAT('{$fname}', '?v=',unix_timestamp(CURRENT_TIMESTAMP)) where meta_field = 'lev' ");
					if(is_file(base_app.$_SESSION['system_info']['lev'])) unlink(base_app.$_SESSION['system_info']['lev']);
				}else{
					$qry = $this->conn->query("INSERT into system_info set meta_value = '{$fname}',meta_field = 'lev' ");
				}
			}
			imagedestroy($temp);
		}
